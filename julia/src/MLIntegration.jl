...
# Try loading common ML libraries, but don't fail if not available
...
const SCIKIT_LEARN_PY_AVAILABLE = try
    using PyCall
    const sklearn_py = PyCall.pyimport("sklearn")
    true
catch
    false
end
...
function optimize_hyperparameters(X, y, ml_model, config::MLHyperConfig)
    ...
    function fitness_function(position)
        ...
        try
            score = cross_validate(model, X, y, config.cv_folds, config.scoring)
...
function set_hyperparameters(model, hyperparams::Dict)
    if SCIKIT_LEARN_PY_AVAILABLE && PyCall.pyisinstance(model, sklearn_py.base.BaseEstimator)
